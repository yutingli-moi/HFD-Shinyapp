#Edit version: 16.04.2025####
#Auto-grab script for HFD
#install packages if you have not

####################
#install.packages("remotes", "HMDHFDplus" )

####update package, current HMDHFDplus version: 2.0.6
#library(remotes)
...
tryCatch({
 ...
 PARITY_data <- bind_rows(parity_list) %>%
  ...
  group_by(code, Cohort, country) %>%                       
   filter(x == max(x)) %>%
 summarise(
    childlessness = l0x / 10000 * 100,     
    parity1 = l1x / 10000 * 100,        
    parity2 = l2x / 10000 * 100,    
    `parity3+` = sum(l3x, l4x, na.rm = TRUE) / 10000 * 100  
  ) %>%
  select(code, Cohort, country, childlessness, parity1, parity2, `parity3+`)  

  PARITY_data_long <- PARITY_data %>%
   pivot_longer(cols = c(childlessness, parity1, parity2, `parity3+`), 
